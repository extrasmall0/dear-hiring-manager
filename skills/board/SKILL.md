...
2. Render a board grouped by `status`, in pipeline order:
   **skipped → filled → submitted → rejected → interview → offer**.
   - A count per column header.
   - One line per application under its status: `company — role  (fit N · applied <date> · N flags)`.
   - A one-line summary: totals, and response rate = `(interview + offer) / submitted`.
...
- Status lifecycle: skipped | filled | submitted | rejected | interview | offer.
